...
def make_label(frame):
    ### 1列目
    # 餌 座標       (基準)
    lbl_Feed_points = ttk.Label(frame, text='Prey Points', font=("Helvetica", FONT_SIZE))
    lbl_Feed_points.grid(row=1, column=0)
    ref_lbl_Feed_points = lbl_Feed_points.grid_info()       # 相対位置指定用
    # 亀 座標
    lbl_turtle_points = ttk.Label(frame, text='Predator Points', font=("Helvetica", FONT_SIZE))
    ...
    ## 領域
    # x
    lbl_x_description = ttk.Label(frame, text='space', font=("Helvetica", FONT_SIZE))
    lbl_x_description.grid(row=ref_lbl_xlim["row"], column=ref_lbl_xlim["column"] + 2, sticky=tk.W)
    # y
    lbl_y_description = ttk.Label(frame, text='space', font=("Helvetica", FONT_SIZE))
    lbl_y_description.grid(row=ref_lbl_ylim["row"], column=ref_lbl_xlim["column"] + 2, sticky=tk.W)
    # 分割
    lbl_grid_sep_description =  ttk.Label(frame, text='space', font=("Helvetica", FONT_SIZE))
    lbl_grid_sep_description.grid(row=ref_lbl_grid_sep["row"], column=ref_lbl_grid_sep["column"] + 2, sticky=tk.W)

    # 視界
    lbl_sight_description = ttk.Label(frame, text='space', font=("Helvetica", FONT_SIZE))
...
def add_new_point(frame, point_list:list, input_area_x:Entry, input_area_y:Entry, cb:ttk.Combobox, row, col):
    ...
    cb_var = tk.StringVar()
    cb = ttk.Combobox(frame, textvariable=cb_var, values=point_list, width=16, font=("Helvetica", FONT_SIZE), justify=tk.RIGHT)
    cb.bind('<<ComboboxSelected>>', lambda e:select_point(point_list, cb_var, input_area_x, input_area_y))
...
def rm_point(frame, point_list:list, input_area_x:Entry, input_area_y:Entry, cb:ttk.Combobox, row, col):
    ...
    if point in point_list:
        ...
        messagebox.showerror("invaild value")
        return
...
def make_input_area(frame, data:TG_Options):
    ...
    # 追加ボタン
    point1_add = ttk.Button(frame, text="add", command=lambda: add_new_point(frame, Feed_points, point1_x_input_area, point1_y_input_area, point1_cb, ref_point1_cb["row"], ref_point1_cb["column"]))
    point1_add.grid(row=ref_point1_cb["row"], column=ref_point1_cb["column"] + 5)
    # 削除ボタン
    point1_rm = ttk.Button(frame, text="remove", command=lambda: rm_point(frame, Feed_points, point1_x_input_area, point1_y_input_area, point1_cb, row=ref_point1_cb["row"], col=ref_point1_cb["column"]))
    ...
    # 追加ボタン
    point2_add = ttk.Button(frame, text="add", command=lambda: add_new_point(frame, Turtle_points, point2_x_input_area, point2_y_input_area, point2_cb, ref_point2_cb["row"], ref_point2_cb["column"]))
    point2_add.grid(row=ref_point2_cb["row"], column=ref_point2_cb["column"] + 5)
    # 削除ボタン
    point2_rm = ttk.Button(frame, text="remove", command=lambda: rm_point(frame, Turtle_points, point2_x_input_area, point2_y_input_area, point2_cb, ref_point2_cb["row"], ref_point2_cb["column"]))
    ...
    ##移動アルゴ
    move_choice = ("Stay", "Random Walk", "Smelly Prey")
    # 餌移動
    ...
    ## 距離
    distance_choice = ("Euclidean metric", "Manhattan metric")
    # 距離関数
    ...
    ## 移動速度
    # スピード
    speed_choice = ["0 (fast)"] + [str(i) for i in reversed(range(2, 11))] + ["1 (slow)"]
    ...
    ## ファイル作成ボタン
    make_bt = ttk.Button(frame, text="Save", 
    ...
    # 実行ボタン
    run_btn = ttk.Button(frame, text="Run", command= lambda :run())
...
def file_make(file_name, Options:TG_Options, Feed_points, Turtle_points, xlim, ylim, sep, sight, delay, move_limit):
    ...
    table = {       # 入力と出力の対応表
        "Stay":"Move.stay", 
        "Random Walk":"Move.RandomWalk", 
        "Smelly Prey": "Move.HaveNose", 
    ...
    if os.path.exists(output_file_path):
        file_overwrap_flag = messagebox.askyesno("warning", "overwrite the file?")
    ...
    if Options.fill_check():
        messagebox.showerror(title="Error", message="Please select all items.")
        return
